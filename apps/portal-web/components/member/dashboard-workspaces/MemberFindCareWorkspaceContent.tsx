...
export type MemberFindCareProvider = {
  id: string;
  name: string;
  specialty: string;
  distance: string;
...
export function MemberFindCareWorkspaceContent({
  embedded = false,
  heroImageSrc = '/assets/portal-images/custom/provider-dashboard-physician-hero.png',
  providers
}: {
  embedded?: boolean;
  heroImageSrc?: string;
  ...
  return (
    <div className="space-y-6">
      ...
      <SurfaceCard title="Provider search" description="Use specialty, location, and access preferences to narrow your options.">
      ...
      <div className="grid gap-4">
        {providers.map((provider) => (
          <SurfaceCard key={provider.id} title={provider.name} description={`${provider.specialty} • ${provider.distance}`}>
