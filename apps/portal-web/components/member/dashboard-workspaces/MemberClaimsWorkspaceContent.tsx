...
export function MemberClaimsWorkspaceContent({
  embedded = false,
  items
}: {
  embedded?: boolean;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-4">
        ...
        <StatCard
          label="Pending items"
          value={String(items.filter((item) => item.status.toLowerCase().includes('pending') || item.status.toLowerCase().includes('review')).length)}
          detail="Claims currently waiting on additional review."
          tone="warning"
        />
        <StatCard
          label="Approved this month"
          value={String(items.filter((item) => item.status.toLowerCase().includes('approved') || item.status.toLowerCase().includes('paid')).length)}
      ...
      <SurfaceCard title="Claims list" description="Use the filter bar to narrow results by status or claim date.">
        <div className="mb-4 grid gap-3 md:grid-cols-[1.3fr_repeat(3,minmax(0,1fr))]">
          <input className="portal-input px-4 py-3 text-sm outline-none" placeholder="Search claims" aria-label="Search claims" />
        ...
        {items.length === 0 ? (
          <EmptyState title="No claims available" description="Claims will appear here when the local member API is available." />
        ) : (
                ...
                {items.map((item) => (
                  <tr key={item.id} className="h-11">
                    <td className="px-4 py-4">
                      <div>
                        <p className="font-semibold text-[var(--text-primary)]">{item.claimNumber}</p>
                        <p className="mt-1 text-[13px] text-[var(--text-muted)]">{item.sourceSystem}</p>
                      </div>
                    </td>
                    <td className="px-4 py-4 text-[var(--text-secondary)]">{formatDate(item.claimDate)}</td>
                    <td className="px-4 py-4">
                      <StatusBadge label={titleCase(item.status)} />
                    </td>
                    <td className="px-4 py-4 text-[var(--text-secondary)]">{formatCurrency(item.totalAmount)}</td>
                    <td className="px-4 py-4 text-right">
                      <Link href={`/dashboard/claims/${item.id}`} className="text-sm font-semibold text-[var(--tenant-primary-color)]">
                        Open
                      </Link>
                    </td>
                  </tr>
